...
void preloop(haier_protocol::ProtocolHandler* handler) {
  smartair2_control_tick(ac_state);
  if (toggle_ac_power) {
    toggle_ac_power = false;
    uint8_t ac_power = ac_state.ac_power;
    ac_state.ac_power = ac_power == 1 ? 0 : 1;
    smartair2_control_note_change("keyboard", "power", ac_power, ac_state.ac_power);
  ...
  if (start_pairing) {
    ...
    smartair2_control_note_change("keyboard", "pairing", 0, 1);
...
int main(int argc, char** argv) {
  if (argc == 2) {
    ...
    smartair2_control_init(ac_state);
